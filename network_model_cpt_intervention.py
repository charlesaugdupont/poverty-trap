...
def simulation(NUM_AGENTS=1250, 
	...
	POISSON_TIMES = np.random.poisson(poisson_scale, size=(NUM_AGENTS, 25))
	...
	# identify poorest 7 percent
	bottom_7_percent = np.argsort(WEALTH[50])[:int(0.07*len(WEALTH[50]))]
	# determine budget (amount of money needed to get entire bottom 7% back to wealth = 1.0)
	BUDGET = sum(1.0 - WEALTH[50][bottom_7_percent])
	for agent in bottom_7_percent:
		WEALTH[50,agent] = stimulus

	# RUN REST OF SIMULATION
	for step in tqdm(range(50,STEPS)):
	...
	return WEALTH, ATTENTION, UTILITIES, ALL_PORTFOLIOS, UPDATE_TIMES, communities, GAMBLE_OBSERVED_SAMPLES, bottom_7_percent
